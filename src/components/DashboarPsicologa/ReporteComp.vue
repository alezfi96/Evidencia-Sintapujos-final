<template>
  ...
  <div class="col-12 col-md-12 col-xl-12">
    ...
    <div class="boton">
      <button @click="descargarListaCitas" class="btn btn-sm btn-outline-secondary">Descargar Lista de Citas</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async descargarListaCitas() {
  try {
    const response = await axios.get("http://localhost:4000/cita/");
    const citas = response.data;
    // Crear contenido PDF
    const pdf = new jsPDF();
    const columns = Object.keys(citas[0]);
    const rows = citas.map(cita => Object.values(cita));
    pdf.autoTable({ columns, body: rows })
    // Descargar archivo PDF
    pdf.save("lista_citas.pdf");

    // Mostrar mensaje de éxito
    Swal.fire({
      position: "top-center",
      icon: "success",
      title: "Lista de citas descargada correctamente.",
      showConfirmButton: false,
      timer: 1500,
    });
  } catch (error) {
  console.log(error);
    Swal.fire({
      position: "top-center",
      icon: "error",
      title: "Error al descargar la lista de citas.",
      showConfirmButton: false,
      timer: 1500,
    });
  }
